...
Level 30: Find the password hidden in a Git tag.	Clone repo. Use git tag to list, then git show [tagname].

Level 31: Push a specific file to the remote repo.	Clone repo. echo "May I come in?" > key.txt. git add, git commit, git push. Then git pull for the new password file.

Level 32: Break out of a restricted shell.	Run a command (e.g., ls). When in the pager (less), type v to enter vim. In vim, type :set shell=/bin/bash then :shell.

Level 33: Use the bandit34-do binary to get the password.	In the new shell: ./bandit34-do cat /etc/bandit_pass/bandit34 (or similar file path). The output is the SSH key.

Level 34: Log in using the SSH key.	Save key to file (chmod 600 key). ssh -i key bandit34@.... The final message is the answer.
